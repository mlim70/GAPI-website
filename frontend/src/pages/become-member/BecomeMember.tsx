...
export default function BecomeMember({ user, setUser }: BecomeMemberProps) {
  ...
  const handleCheckout = async (levelKey: string, formData: any) => {
    ...
    try {
      ...
      // Store email in sessionStorage so the verification page can show it without exposing it in the URL
      sessionStorage.setItem('pendingVerificationEmail', formData.email);
      // Redirect to email verification page (no email= in URL — reduces phishing signals)
                  ...
                  <div className="p-6 flex flex-col h-full">
                    ...
                    <div className="mt-auto">
                      <button
                        ...
                        className={`w-full font-medium py-3 px-4 rounded-md transition-colors duration-200 flex items-center justify-center ${isCurrentLevel(level)
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
              ...
              >
                ...
                <div className="w-12 h-12 flex items-center justify-center">
                  <div className={`w-0 h-0 border-l-[8px] border-r-[8px] border-t-[12px] border-l-transparent border-r-transparent border-t-gray-400 group-hover:border-t-gray-600 transition-all duration-300 transform origin-center ${showHowItWorks ? 'rotate-180' : 'rotate-0'
                    }`}></div>
                </div>
              </button>

              {/* Dropdown Content */}
              <div className={`overflow-hidden transition-all duration-500 ease-in-out ${showHowItWorks ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'
                }`}>
