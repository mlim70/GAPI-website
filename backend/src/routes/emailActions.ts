...
import { updateUserPassword } from '../utils/email/userVerification';
...
router.post('/reset-password',
  ...
  async (req, res) => {
    try {
      ...
      // Hash the incoming token and look up the user directly by it
      const hash = crypto.createHash('sha256').update(token).digest('hex');
      const user = await User.findOne({ resetTokenHash: hash }).select('+resetTokenHash +resetTokenExpires');
      ...
      // Check if reset token is still within its expiry window
      if (!user.resetTokenExpires || user.resetTokenExpires < new Date()) {
        return res.status(400).json({ error: 'expired' });
      }

      // Update password and clear reset token
      const dbResult = await updateUserPassword(user._id.toString(), newPassword);
